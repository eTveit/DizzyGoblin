...
public class MovementGoblin : MonoBehaviour {
    ...
    void Update() {
        ...
        //This system should work just as well with keyboard as with controller
        if(newSystem) {
            float xMove = Input.GetAxis("Horizontal");
            float zMove = Input.GetAxis("Vertical");

            float moveSum = Mathf.Abs(xMove) + Mathf.Abs(zMove);

            //Restrain to keep goblin from moving faster when moving diagonally
            if(moveSum > 1) {
                xMove /= moveSum;
                zMove /= moveSum;
            }

            x -= xMove * speed * Time.deltaTime;
            z -= zMove * speed * Time.deltaTime;
